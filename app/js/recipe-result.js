const data = JSON.parse(localStorage.getItem("fetchData") || "[]");
favoritesLabels = [];
favorites = [];
mealPlan = [];
...
data.hits.forEach((recipe, i) => {
  renderRecipeCards(recipe, i);
});
    ...
    if (thisBtn === btn) {
      if (thisBtn.firstChild.className === "fa-regular fa-star") {
        ...
        const obj = favorites.find(
          (item) => item.recipe.label === data.hits[i].recipe.label
        );
...
submitBtns.forEach((btn, i) => {
  btn.addEventListener("click", () => {
    ...
    const obj = { date: dateInputs[i].value, ...data.hits[i] };
